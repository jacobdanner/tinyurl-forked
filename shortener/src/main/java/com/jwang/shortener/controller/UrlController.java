...
public class UrlController {
    ...
    public Map<String, Object> createShortUrl(
            @RequestBody Map<String, String> reqBody,
            @RequestHeader(name = "Authorization") String requestTokenHeader) {
        ...
        if (requestTokenHeader != null && requestTokenHeader.startsWith("Bearer ")) {
            ...
            try {
                ...
                throw new ResponseStatusException(
                        HttpStatus.UNAUTHORIZED, "session timeout, please log in again");
            } catch (Exception e) {
                throw new ResponseStatusException(
                        HttpStatus.UNAUTHORIZED, "unknown error occurred, please try again");
            }
        }
        ...
        if (!reqBody.containsKey("original")) {
            throw new ResponseStatusException(
                    HttpStatus.BAD_REQUEST, "field original must be present!");
        }
        ...
        if (reqBody.containsKey("expireDate") && reqBody.get("expireDate") != null) {
            ...
            if (expireDate.isBefore(LocalDate.now())) {
                throw new ResponseStatusException(
                        HttpStatus.BAD_REQUEST, "expiration date is invalid!");
            }
        } else {
            // default to one year
        ...
        // validate alias
        if (reqBody.containsKey("alias")
                && reqBody.get("alias") != null
                && reqBody.get("alias").trim().length() > 0) {
            String alias = reqBody.get("alias").trim();
            if (alias.length() < 6
                    || alias.length() > 15
                    || UrlValidator.getInstance().isValid(alias)) {
                throw new ResponseStatusException(
                        HttpStatus.BAD_REQUEST,
                        "alias size must be between 6 and 15 and not a url");
            }
            if (alias.matches("^.*[^a-zA-Z0-9 ].*$")) {
                throw new ResponseStatusException(
                        HttpStatus.BAD_REQUEST, "alias must be alphanumeric");
            }
            shortenUrl = urlService.customizeShortenUrl(originalUrl, alias, expireDate, username);
            if (shortenUrl == null) {
                throw new ResponseStatusException(
                        HttpStatus.BAD_REQUEST, "alias in use, try another one");
            }
        } else {
